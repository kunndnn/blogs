    ...
    return (
        <div className="hidden lg:block lg:col-span-4 sticky top-24 space-y-6 h-fit pl-8 border-l border-border-soft">
            <h3 className="text-xl font-bold mb-4 text-text-main flex items-center gap-2">
                <span className="w-1.5 h-6 bg-brand-primary rounded-full shadow-[0_0_12px_rgba(99,102,241,0.5)]" />
                Quick Steps
            </h3>
                ...
                {steps?.map(({ name }, index) => (
                    <div key={index} className="flex gap-4 group">
                        ...
                        <button
                            ...
                            className="text-text-muted group-hover:text-text-main hover:cursor-pointer text-left transition-all font-medium text-sm leading-relaxed w-full bg-transparent border-none p-0"
                        >
                            {convert(name)}
                        </button>
                    </div>
